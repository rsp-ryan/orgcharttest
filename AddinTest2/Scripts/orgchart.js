...
(function ($) {
	...
	$.fn.jOrgChart.defaults = {
		...
		nodeClicked: function ($node, type) {
		    var selectedID = $node.clone().children("ul,li").remove().end().html();
		    selectedID = selectedID.replace("<span><div class=\"", "");
		    selectedID = selectedID.substring(0, selectedID.indexOf("\">"));
		    alert(selectedID);
		}
	};
	...
	function wrapContent(content) {
		...
		var wrapper = $("<span>");
		var appendStr = teamNames[content];
		var contentDiv = $("<div>").addClass(content).append(appendStr);
